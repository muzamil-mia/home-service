...
public class VendorsService {
	...
	public ResponseEntity<ResponseStructure<Vendors>> saveVendors(Vendors vendor) {
		// why we are creating this object? to initialize it with response structure
		// class data.
	...
	public ResponseStructure<Vendors> login(Vendors vendor) {
        ResponseStructure<Vendors> structure = new ResponseStructure<>();
        try {
            Vendors vendors = dao.getVendorByEmail(vendor.getEmail());
            if (vendors != null) {
                if (vendors.getPassword().equals(vendor.getPassword())) {
                    structure.setData(vendors);
                    structure.setStatus(HttpStatus.OK.value()); // 200 OK
                    structure.setMessage("Login successful");
                } else {
                    structure.setStatus(HttpStatus.UNAUTHORIZED.value()); // 401 Unauthorized
                    structure.setMessage("Wrong password. Please enter the correct one.");
                }
            } else {
                structure.setStatus(HttpStatus.NOT_FOUND.value()); // 404 Not Found
                structure.setMessage("Email not found.");
            }
        } catch (Exception e) {
            structure.setStatus(HttpStatus.INTERNAL_SERVER_ERROR.value()); // 500 Internal Server Error
            structure.setMessage("An unexpected error occurred.");
        }
        return structure;
    }

	public ResponseEntity<ResponseStructure<Vendors>> registerVendor(Vendors vendor) {
		ResponseStructure<Vendors> structure = new ResponseStructure<Vendors>();
		try {
			Vendors vendo = dao.registerVendor(vendor);
			structure.setData(vendo);
			structure.setMessage("vendor registered successfully");
			structure.setStatus(HttpStatus.CREATED.value());
			...
			structure.setMessage("An error occurred while registering the vendor.");
